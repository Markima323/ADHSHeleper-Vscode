...
- 在可拖动的行解释悬浮窗底部加入单行继续提问框，支持 Enter 发送。
- 后续回答显示在首次解释下方，并携带该行代码、首次解释和最近 6 轮问答作为上下文。
- Gemini 与 DeepSeek 均支持行级追问，切换 AI 时仍会丢弃旧服务的迟到响应。

## 0.6.1

- 开始学习时自动收起左侧主侧边栏，并关闭作为学习来源的代码文件标签。
- 学习卡片在合并后的单一编辑器区域打开，不再创建保留源码的并排编辑器组。
- 未保存文件仍由 VS Code 显示正常的保存确认，不会强制丢弃修改。
